...
export default class MastersPortalPlaywrightCrawler extends PlaywrightBaseCrawler {
    ...
    /**
     * Extract program data from Masters Portal page
    ...
    /**
     * Override to add custom filtering for Masters Portal URLs
    ...
    shouldCrawlUrl(url) {
        ...
        // Clean URL by removing hash fragments and ref parameters for comparison
        const cleanUrl = url.split('#')[0].split('?')[0];

        // Only allow two types of URLs:
        // 1. Study pages: /studies/[id]/[program-name].html
        // 2. Search/pagination pages: /search/master or /search/master?page=X
        const allowedPatterns = [
            /\/studies\/\d+\/[^/]+\.html/,  // Study pages
            /\/search\/master$/              // Search page (exact match)
        ];
