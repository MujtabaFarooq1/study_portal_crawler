...
export default class PlaywrightBaseCrawler {
    ...
    constructor(config = {}) {
        ...
        this.headless = config.headless !== false; // Default to true
    ...
    async processPage(url) {
        ...
        const pageType = isStudyPage ? '[STUDY]' : isSearchPage ? '[SEARCH]' : '[OTHER]';

        console.log(`\n${'='.repeat(80)}`);
        console.log(`[${this.crawledCount}/${this.maxCrawlLength}] ${pageType} Crawling:`);
        console.log(`URL: ${url}`);
        ...
        try {
            ...
            if (data && data.length > 0) {
                ...
                snapshot.forEach((item, index) => {
                    ...
                    if (item.university) console.log(`     University: ${item.university}`);
                    if (item.location) console.log(`     Location: ${item.location}`);
                    if (item.link) console.log(`     Link: ${item.link.substring(0, 60)}...`);
                });
            ...
            if (links.length > 0) {
                ...
                const searchLinks = links.filter(l => /\/search\/(master|bachelor)/.test(l));
            ...
            console.log(`⏱️  Waiting ${Math.floor(this.requestDelay / 1000)}s before next request...\n`);
